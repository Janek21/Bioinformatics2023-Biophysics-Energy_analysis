...
####STEP 1
from step1_2 import *
dt=7 #quin es el distance treshold?
...
res_lib = ResiduesDataLib('./Biophysics_A1/assignment_data/parameters_vanderw.txt????????????????????????????????????????????????????????????????????')
# loading VdW parameters
ff_params = VdwParamset('./Biophysics_A1/assignment_data/parameters_step2.txt????????????????????????????????????????????????????????????????????????')
# set the pdb_path and load the structure
pdb_path = "./Biophysics_A1/assignment_data/6m0j_fixed.pdb"
